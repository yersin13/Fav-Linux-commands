...
# ---------------------------------------------------------------------------
# BEGINNER NOTES — one plain-english sentence shown before the technical desc
# ---------------------------------------------------------------------------
BEGINNER_NOTES = {
    "mkfifo":   "Think of this as a two-way pipe between your keyboard and a remote machine — the building block of the classic reverse shell.",
    "nc":       "Netcat is the duct tape of networking — it can open a raw connection to any port, send files, and give you a shell on a remote machine.",
    "dd":       "Like a photocopier for your hard drive — copies everything bit-by-bit, including deleted files that the filesystem thinks are gone.",
    "base64":   "Transforms any binary data into readable text characters. Not encryption — anyone can decode it instantly. Used to sneak payloads past filters that block binary data.",
    "xxd":      "Shows you exactly what is inside a file as raw bytes (hex). Essential for spotting hidden data, understanding file formats, and inspecting shellcode.",
    "chroot":   "Changes what '/' (root) means for a process, giving it a fake filesystem to live in. Escaping this fake root is a classic privilege escalation technique.",
    "shred":    "Overwrites a file's data multiple times before deleting it, so forensic recovery tools cannot reconstruct what was there.",
    "chmod":    "Controls who can read, write, or execute a file. The SUID bit is the important one for security — it lets a file run as its owner (often root) regardless of who launches it.",
    "sudo":     "Grants a regular user temporary root (admin) powers for one command. Attackers always run 'sudo -l' first to see which commands they are allowed to run as root.",
    "find":     "Searches the entire filesystem. The '-exec' flag is what makes it dangerous — it can run any command on every file it finds, turning a search tool into a code-execution primitive.",
    "tee":      "Reads from stdin and writes to both a file and stdout at once. Because it writes files, 'sudo tee' is a classic way to overwrite protected system files.",
    "strace":   "Watches every system call a program makes in real time — like reading its diary. Useful for extracting hardcoded passwords, finding hidden config files, and understanding binary behavior.",
    "su":       "Switches your current user to another, including root. Requires the target user's password unless you are already root.",
    "nohup":    "Keeps a process running after you log out by detaching it from the terminal. Attackers use it to leave background processes running persistently.",
    "kill":     "Sends a signal to a process — most commonly signal 9 (SIGKILL) which forces immediate termination. Root can kill any process; regular users can only kill their own.",
    "base32":   "Same idea as base64 but uses a different alphabet. Less recognized by security tools than base64, which is exactly why it is useful for evasion.",
    "od":       "Shows file contents in various numeric formats (octal, hex, decimal). Older than xxd but still useful for inspecting raw binary data.",
    "chcon":    "Changes the SELinux security context label on a file — relevant when trying to bypass SELinux mandatory access controls.",
    "runcon":   "Runs a command under a different SELinux security context. Understanding this helps you see how SELinux can be abused or bypassed.",
    "install":  "Copies files and sets permissions in one step. The ability to set the SUID bit during install makes it a privilege escalation vector if available via sudo.",
    "split":    "Cuts a large file into smaller pieces. Attackers use this to break large payloads or exfiltrated data into chunks that bypass size-based detection.",
    "truncate":  "Changes a file's size — can empty a log file instantly without deleting it, which looks less suspicious than deletion.",
    "dd":       "Like a photocopier for your hard drive — copies everything bit-by-bit, including deleted files that the filesystem thinks are gone.",
    "ssh":      "The standard encrypted remote shell. Beyond just logging in, it can tunnel other protocols through its connection — useful for reaching machines that are not directly accessible.",
    "curl":     "Fetches content from URLs from the command line. Supports dozens of protocols and can send data, follow redirects, and handle cookies — making it useful far beyond just downloading files.",
    "nmap":     "Scans a network to find which hosts are live and which ports are open. The starting point of almost every penetration test.",
    "python3":  "A general-purpose scripting language available on most Linux systems. Its presence means an attacker can run arbitrary scripts, spin up HTTP servers, and escape to a shell if it is in the sudo list.",
    "openssl":  "The command-line tool for everything cryptographic — inspecting certificates, generating keys, encrypting files, and testing TLS connections.",
    "socat":    "An advanced version of netcat that supports TLS encryption, bidirectional streams, and complex relay setups. Preferred for encrypted reverse shells.",
    "tcpdump":  "Captures network packets passing through an interface. On a compromised machine, a brief tcpdump run can collect cleartext passwords and session tokens from adjacent hosts.",
    "awk":      "Processes text line by line, splitting it into fields. Extremely powerful for parsing log files, extracting specific columns, and formatting data.",
    "sed":      "Edits text streams — find and replace, delete lines, extract ranges. The '-i' flag edits files in place, which is what makes it dangerous.",
    "wget":     "Downloads files from URLs. Simpler than curl but present on almost every Linux system. Often used to fetch payloads onto a compromised machine.",
    "john":     "Offline password cracker. Given a file of password hashes, it tries millions of guesses using wordlists and rules until it finds the original passwords.",
    "hydra":    "Online brute-force tool. Unlike John, it attacks live services (SSH, web logins, FTP) by trying username/password combinations over the network.",
    "sudo":     "Grants a regular user temporary root (admin) powers for one command. Attackers always run 'sudo -l' first to see which commands they are allowed to run as root.",
    "timeout":  "Runs a command and kills it after a time limit. Attackers use it to make payloads self-destruct or to test time-based sandbox detection.",
    "env":      "Prints environment variables or runs a command with a modified environment. Environment variables often contain secrets like API keys and database passwords.",
    "stdbuf":   "Changes the buffering behavior of a program's output streams. Used to make output appear in real time and as a behavioral evasion technique.",
    "cat":      "Prints file contents to the terminal. Simple but important — reading /etc/passwd, /etc/shadow, or any credential file starts here.",
    "grep":     "Searches for patterns in text. In security contexts it is used to hunt for passwords, API keys, IP addresses, and indicators of compromise across large volumes of text.",
    "echo":     "Prints text to the terminal. At the heart of the Shellshock vulnerability — injecting commands into an echo call through environment variables was the attack.",
    "ln":       "Creates links between files. Hard links and symbolic links can be used to trick privileged processes into reading or writing files they should not touch.",
    "chmod":    "Controls who can read, write, or execute a file. The SUID bit is the important one for security — it lets a file run as its owner (often root) regardless of who launches it.",
}

# ---------------------------------------------------------------------------
# PLAIN INTENT — ELI5 version of security_intent, plain english for beginners
# ---------------------------------------------------------------------------
PLAIN_INTENT = {
    "xxd":      "A tool that shows you the raw contents of any file as numbers (hex). Security researchers use it to look inside malware, find hidden data, and craft custom payloads byte by byte.",
    "chroot":   "Changes the 'root' of the filesystem for a process, trapping it in a folder. Attackers study how to escape these traps because the same technique is used in containers and sandboxes.",
    "mkfifo":   "Creates a special file that acts like a two-way pipe. The most common use in security is building a reverse shell — a connection that lets an attacker type commands on your machine from theirs.",
    "dd":       "Reads and writes raw data directly, bypassing normal file system rules. Attackers use it to copy entire disks, wipe evidence, or create bootkit payloads that survive reinstalls.",
    "shred":    "Destroys a file so thoroughly that even forensic tools cannot recover it. Attackers run this after a breach to erase their tracks before leaving.",
    "base64":   "Converts data into a text format made of letters and numbers. It is not encryption — think of it as packing data into a box that looks like random text to sneak past simple content filters.",
    "base32":   "Same as base64 but uses a different set of characters. Less well-known, so security tools are less likely to flag it — useful for hiding payloads in plain sight.",
    "nc":       "Opens a raw network connection between two machines. In the wrong hands it is a one-line remote shell. Seeing netcat on a server is always a warning sign.",
    "tee":      "Sends the same data to two places at once — to the screen and to a file. The trick is that 'sudo tee' can write to protected files that you normally cannot touch.",
    "su":       "Lets you become a different user without logging out. The most important target is root. If you know the root password (or have sudo access), one command gives you full control of the system.",
    "nohup":    "Makes a process immune to hangups — it keeps running even after you close the terminal. Attackers use it to plant long-running background processes that survive logout.",
    "kill":     "Stops a running process. Root can stop any process — including security tools, logging daemons, and monitoring agents — which is why privilege escalation matters.",
    "strace":   "Eavesdrops on every request a program makes to the operating system. You can watch it open files, make network connections, and handle passwords — all in real time.",
    "chcon":    "Changes the security label SELinux puts on a file. SELinux is an extra layer of access control; changing these labels is one way to bypass it.",
    "runcon":   "Runs a program with a different SELinux security label. Understanding it helps you see how Linux's mandatory access controls can be circumvented.",
    "install":  "A 'cp with superpowers' that copies files and sets their permissions in one step. The danger is the ability to set the SUID bit — making a copy of a program that runs as root.",
    "split":    "Breaks one big file into many smaller ones. Used to cut large stolen datasets into pieces small enough to pass through email filters or upload size limits without triggering alarms.",
    "truncate":  "Instantly empties a file without deleting it. Attackers use it to wipe log files — the file still exists (so nothing looks missing) but its contents are gone.",
    "timeout":  "Runs a command and automatically kills it after a set time. Used to make malware self-destruct after a window, or to test whether a sandbox exits payloads early.",
    "stdbuf":   "Changes how a program buffers (delays) its output. Used to keep output flowing in real time during an attack, and as a trick to make malicious processes look like normal background jobs.",
    "od":       "Shows the raw bytes of a file in different number formats. Older than xxd but useful for inspecting binary files and spotting patterns that are invisible in plain text.",
    "ssh":      "The standard way to remotely control a Linux server over an encrypted connection. Beyond login, it can create secret tunnels that forward traffic to machines hidden deep inside a network.",
    "curl":     "A command-line tool that can talk to any server using any protocol. Attackers use it to download payloads, test web vulnerabilities, talk to command-and-control servers, and exfiltrate data.",
    "nmap":     "Scans a network to find which computers are reachable and what services they are running. It is the first thing a pentester runs — and the first thing an attacker runs too.",
    "openssl":  "The Swiss army knife of cryptography on the command line. Used to inspect website certificates, create encrypted tunnels, generate keys, and test for vulnerabilities in TLS connections.",
    "socat":    "A more powerful version of netcat that supports encryption. Attackers prefer it for reverse shells because the traffic looks like normal HTTPS and bypasses most network monitoring.",
    "python3":  "A scripting language that turns a compromised machine into a platform for running complex attacks. One line can spin up a web server, another can give you a full remote shell.",
    "tcpdump":  "Captures every packet passing through a network interface. On a compromised internal machine, a short capture can harvest usernames and passwords sent by other computers on the same network.",
    "find":     "Searches the entire filesystem for files matching criteria you specify. The '-exec' flag is the dangerous part — it runs any command on every file found, turning a search into an attack.",
    "awk":      "Processes text by breaking each line into columns and letting you do things with them. Used heavily for parsing log files, extracting credentials from dumps, and formatting attack output.",
    "sed":      "Finds and replaces text in files or streams. The in-place edit flag ('-i') is what matters for attacks — it can silently modify config files, patch binaries, or scrub evidence from logs.",
    "wget":     "Downloads files from the internet onto the current machine. The simplest way to fetch a payload onto a compromised server — one line, no dependencies required.",
    "john":     "Takes a file full of password hashes (scrambled passwords) and tries to reverse them using millions of guesses. The most common use is cracking /etc/shadow after gaining access to a Linux system.",
    "hydra":    "Tries thousands of username/password combinations against a live service until one works. The difference from John is that Hydra attacks over the network in real time.",
    "sudo":     "Lets a regular user run specific commands as root (the all-powerful admin). The most important command after getting access to a system — 'sudo -l' shows you exactly what you can do.",
    "grep":     "Searches files for text patterns. In security, it is used to hunt for passwords, API keys, IP addresses, and other sensitive strings hidden inside config files and logs.",
    "cat":      "Prints a file's contents. Simple but foundational — reading /etc/passwd to list users, reading /etc/shadow for password hashes, and reading config files for credentials all start with cat.",
    "echo":     "Prints text to the terminal. It was at the center of the Shellshock vulnerability — specially crafted environment variables caused echo to execute injected commands on web servers.",
    "chmod":    "Sets who is allowed to read, write, or run a file. The SUID permission bit is the critical one — it causes a program to run as its owner (often root) no matter who actually runs it.",
    "sha256sum": "Produces a unique fingerprint (hash) for a file. Used to verify that a downloaded file has not been tampered with, and in security to check if a file matches a known malware sample.",
    "md5sum":   "Produces a hash fingerprint for a file — less secure than sha256sum but still widely used. Attackers check file hashes to confirm payloads arrived intact and to identify malware variants.",
    "kill":     "Sends a signal to stop a running process. If you have root privileges, you can stop any process — including antivirus software, logging agents, and monitoring daemons.",
    "top":      "Shows which processes are running and how much CPU and memory they are using. During incident response, an unexpected process in the top list is often the first sign of compromise.",
    "hostname":  "Shows or sets the machine's network name. During reconnaissance, running hostname tells you where you are on the network — useful for orienting yourself after landing on a new machine.",
    "whoami":   "Prints your current username. The first command anyone runs after getting access to a system — you need to know whether you are a regular user or already root.",
    "env":      "Lists all environment variables currently set. These often contain API keys, database passwords, and authentication tokens that are passed between applications.",
    "ln":       "Creates links — shortcuts — to files. Symbolic link attacks trick a privileged program into reading or writing a different file than it thinks it is accessing.",
}
...
def get_meta(command):
    ...
    return {
        **base,
        ...
        "beginner_note": BEGINNER_NOTES.get(key, ""),
        "plain_intent": PLAIN_INTENT.get(key, ""),
    }
...
def build_extra_tool(command, data):
    return {
        ...
        "beginner_note": BEGINNER_NOTES.get(command.lower(), ""),
        "plain_intent": PLAIN_INTENT.get(command.lower(), ""),
        "transcript": "",
        "video_url": "",
        "extra": True,
    }
...
def main():
    ...
    for i, v in enumerate(raw_videos):
        ...
        videos.append({
            ...
            "plain_intent": meta["plain_intent"],
